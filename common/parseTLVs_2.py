...
def parsePointCloudTLV(tlvData, tlvLength, outputDict):
    ...
    try:
        # 1. Check if the length makes sense for the struct size
        if tlvLength % pointStructSize != 0:
            print(f"Skipping frame: TLV length {tlvLength} is not a multiple of {pointStructSize}")
            return
        ...
        print(f"Parser hit a bad byte: {e}. Skipping this frame.")
        # Clear the dict so the rest of the script doesn't try to use old data
...
def parseSideInfoTLV(tlvData, tlvLength, outputDict):
    ...
    pointStructSize = struct.calcsize(pointStruct)
    numPoints = int(tlvLength/pointStructSize)
    struct_size = struct.calcsize(pointStruct)
    numIncomingInfo = tlvLength // struct_size

    # CRITICAL FIX: Only loop up to the smaller of the two numbers
    # This prevents the "Index Out of Bounds" crash
    for i in range(min(numIncomingInfo, numPoints)):
        try:
        ...
        # SNR and Noise are sent as uint16_t which are measured in 0.1 dB Steps
            ...
            print(f"Error unpacking side info at index {i}")
            break
...
def parseTrackTLV(tlvData, tlvLength, outputDict):
    print(tlvLength)
    targetStruct =  "I9f" #'I27f'
    targetSize = struct.calcsize(targetStruct)
    targets = np.array([])
    try:
        # 1. Check if the length makes sense for the struct size
        if tlvLength % targetSize != 0:
            print(f"Skipping frame: TLV length {tlvLength} is not a multiple of {targetSize}")
            return
        
        numDetectedTargets = int(tlvLength/targetSize)
        if numDetectedTargets > 20 or numDetectedTargets < 0:
            print(f"⚠️ Skipping Corrupt Frame: Impossible target count ({numDetectedTargets})")
            return # This stops the function before NumPy crashes
        ...
        for i in range(numDetectedTargets):
            ...
            targets[i, 0] = targetData[0] # Target ID (TID)
            targets[i, 1] = targetData[1] # X Position (m)
            targets[i, 2] = targetData[2] # Y Position (m)
            targets[i, 3] = targetData[3] # Z Position (m)
            targets[i, 4] = targetData[4] # X Velocity (m/s)
            targets[i, 5] = targetData[5] # Y Velocity (m/s)
            targets[i, 6] = targetData[6] # Z Velocity (m/s)
            targets[i, 7] = targetData[7] # X Acceleration (m/s^2)
            targets[i, 8] = targetData[8] # Y Acceleration (m/s^2)
            targets[i, 9] = targetData[9] # Z Acceleration (m/s^2)
...
def parseIDInfo(tlvData, tlvLength, outputDict):
    #print(tlvData)
    if len(tlvData) < 4:
        print(f"⚠️ Warning: IDInfo TLV too short ({len(tlvData)} bytes). Skipping.")
        return
    ...
    try:
        # 1. Check if the length makes sense for the struct size
        if numBoxes > 20:
            print(f"⚠️ Warning: Corrupt IDInfo detected (numBoxes={numBoxes}). Skipping.")
            return
        
        if tlvLength % numBoxes != 0:
            print(f"Skipping frame: TLV length {tlvLength} is not a multiple of {numBoxes}")
            return
...
def parseStatsInfo (tlvData, tlvLength, outputDict):
    ...
    if len(tlvData) < 24:
        # print(f"⚠️ Warning: Stats TLV too short ({len(tlvData)} bytes). Skipping.")
        return
    try:
        ...
        print(f"❌ Error in parseStatsInfo: {e}")
