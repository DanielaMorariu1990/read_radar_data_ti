...
def parseStandardFrame(frameData, demo=None):
    ...
    try:
        ...
        print('error could not read the frame header')
    ...
    for i in range(numTLVs):
        try:
            ...
            print(tlvType)
            print(f"len tlv type:{tlvLength}")
            ...
            return {}
        # if tlvType == 1021:
        #     parseTrackTLV(frameData[:tlvLength], tlvLength, outputDict)
        if (tlvType in parserFunctions):
           print(f"for {tlvType} and {tlvLength} we use function {parserFunctions[tlvType]}")
